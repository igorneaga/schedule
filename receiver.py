...
class DataProcessor:
    ...
    def main_program_order(self):
        try:
            ...
            if self.number_close_trials > 2:
                ...
                msg_response = messagebox.showerror("Close File", "Save and close excel files to continue... \n\n"
                                                    + str(p))
                if msg_response == "ok":
                    self.number_close_trials += 1
                    self.main_program_order()

    def create_excel_copies(self):
        """Will create a copy of original files and will keep it in the folder."""

        if not os.path.exists('copy_folder'):
            os.makedirs('copy_folder')

        if not self.file_directory:
            messagebox.showerror("Error Box", "An error happened. A file is not found. \nPlease report the issue.")
        else:
            # Main Program Logic
    ...
    def iter_rows(self, read_worksheet, file_path, sheet_name):
        """ Loops through rows. Checks if the value is none. And transforms time to the proper format """
    ...
    def color_comment_copy_excel(self):
        ...
        for file_path in self.list_file_paths:
            get_excel_workbook = openpyxl.load_workbook(file_path)
            ...
            for i in range(len(self.user_excel_errors)):
                if self.user_excel_errors[i].get("File_Path") == file_path:
                    ...
                    # del copy_user_excel_errors[i]

            get_excel_workbook.save(file_path)
            ...
            for val_length in range(len(none_index)):
                ...
                if column > 13:
                    ...
                    try:
                        ...
                        print("Index Error at mark_none_values")
                        pass
    ...
    def set_dict_courses(self, excel_data):
        ...
        def course_room_format(room_number):
            ...
            filter_str_list = list(filter(None, str_list))  # Removes empty str
        ...
        def set_online_course(d_courses, j):
        ...
        for j in excel_data:
            try:
                ...
                """
                if j[self.excel_course_time] == 'None':
                    comment = 'If it is Online course - enter it, if you want to keep it empty - enter "N/A"'
                    self.create_report_dictionary(j[8], 6, j[10], j[9], error_color="FF687B", error_comment=comment)
                """
                ...
                if dict_courses.get("Room") == "ONLINE":
                    ...
                    try:
                        ...
                        # Due to space table concern, we need to limit evening classes
                        if time_split[0][1] == '6':
                    ...
                    elif time_split[0] == "Online":
                        dict_courses["Course_Days"] = ["Online"]
                            ...
                            if list_dict[ig].get('Room') == 'Online' or list_dict[ig].get("Start_Time") == 'Online':
                                ...
                                if start_time_i is None or start_time_d is None:
                                    ...
                                    if (list_dict[ig].get('Room') is not None) & \
                                        ...
                                        # It is normal for section 40 to conflict with another course
                                        if (section_numb_d[1] == '40') or (section_numb_ig[1] == '40'):
                                            ...
                                            if start_time_d <= start_time_i <= end_time_d:
                                                ...
                                                if fifteenth_minutes_start_d <= fifteenth_minutes_start_i <= fifteenth_minutes_end_d:
                                                    self.time_conflict_comment(list_dict[ig], list_dict[d + 1], True)
                                
                                                else:
                                                    if fifteenth_minutes_start_d <= fifteenth_minutes_end_d <= fifteenth_minutes_end_i:
                                                        self.time_conflict_comment(list_dict[ig],
                                                                                   list_dict[d + 1], True)
                                                """
    ...
    def time_conflict_comment(self, first_dict, second_dict, fifteenth_conflict=False):
        """
        get_sheet_name = second_dict.get("Sheet_Name")
        get_file_path = second_dict.get("File")[12:]

        if len(get_sheet_name) > 10:
            get_sheet_name = get_sheet_name[:10] + "..."
        if len(get_file_path) > 14:
            get_file_path = get_file_path[:14] + "..."
        """
    ...
    def create_excel_table(self):
        self.set_course_type()
        print(self.list_dict_courses)
